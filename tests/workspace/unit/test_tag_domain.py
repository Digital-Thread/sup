...
@pytest.fixture()
def tag_minimal(workspace_id: WorkspaceId) -> Tag:
    return Tag(
...
@pytest.fixture()
def tag_full(workspace_id: WorkspaceId) -> Tag:
    return Tag(_workspace_id=workspace_id, _name='Full Tag', _color='#FAFAFA', _id=TagId(1))


class TestTagCreation:

    def test_tag_creation_with_required_data(
        self, tag_minimal: Tag, workspace_id: WorkspaceId
    ) -> None:
    ...
    def test_tag_creation_with_full_data(self, tag_full: Tag, workspace_id: WorkspaceId) -> None:
...
class TestTagValidation:
    ...
    def test_valid_tag_name(self, tag_minimal: Tag, name: str) -> None:
        tag_minimal.name = name
        assert tag_minimal.name == name

    @pytest.mark.parametrize('color', ['#A0A0A0', '#ffffff', '#000000'])
    def test_valid_tag_color(self, tag_minimal: Tag, color: str) -> None:
    ...
    def test_invalid_tag_name(
        self, tag_minimal: Tag, name: str, expected_error_message: str
    ) -> None:
        with pytest.raises(ValueError, match=expected_error_message):
            tag_minimal.name = name

    @pytest.mark.parametrize('color', ['', ' ', '#-%&*()+', '000000', 'FAFAFA'])
    def test_invalid_tag_color(self, tag_minimal: Tag, color: str) -> None:
