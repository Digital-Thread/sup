...
from src.data_access.models.workspace_models import (
    RoleModel,
...
class UserWorkspaceRoleRepository(IUserWorkspaceRoleRepository):
    ...
    async def assign_role_to_workspace_member(
        ...
        await self._check_member_in_workspace(member_id, workspace_id)
        await self._check_exists_role_in_workspace(role_id, workspace_id)

        is_exists_member_role = await self._is_exists_role_for_member_in_workspace(
    ...
    async def _check_member_in_workspace(
    ...
    async def _check_exists_role_in_workspace(
    ...
    async def _is_exists_role_for_member_in_workspace(
        self, member_id: MemberId, workspace_id: WorkspaceId
    ) -> bool:
    ...
    async def remove_role_from_workspace_member(
        ...
        if is_exists_member_role:
            await self._session.execute(
                delete(UserWorkspaceRoleModel).filter_by(
                    user_id=member_id, workspace_id=workspace_id
                )
            )
        else:
            await self._check_exists_workspace(workspace_id)
