...
from src.apps.workspace.exceptions.category_exceptions import CategoryException, CategoryNotFound
...
class GetCategoryByIdInteractor:
    ...
    async def execute(self, category_id: int, workspace_id: UUID):
        try:
            category = await self._category_repository.find_by_id(CategoryId(category_id), WorkspaceId(workspace_id))
